...
for node in tree.body:
    if isinstance(node, ast.FunctionDef) and node.name in functions_to_extract:
        # Use ast.get_source_segment or compile/exec the node
        # But simple string extraction is easier if we have the source
        start = node.lineno - 1
        end = node.end_lineno
        # Adjust for decorators if any (not critical here as we don't have many)
...
# Call the function from exec_globals
...
if "usuarios" in xls_debug.sheet_names:
    print("DEBUG: Users sheet columns:", pd.read_excel(xls_debug, "usuarios").columns.tolist())
    print("DEBUG: Users sheet data:", pd.read_excel(xls_debug, "usuarios").to_dict(orient="records"))
...
header_match = (
    str(json_out["numDocumentoIdObligado"]) == str(sample_json["numDocumentoIdObligado"]) and
    str(json_out["numFactura"]) == str(sample_json["numFactura"])
)
print(f"Header match: {header_match}")
if not header_match:
    print(f"Header Expected: {sample_json['numDocumentoIdObligado']}, Got: {json_out['numDocumentoIdObligado']}")
...
dob_match = (
    str(json_out["usuarios"][0]["fechaNacimiento"]) == str(sample_json["usuarios"][0]["Fechanacimiento"])
)
